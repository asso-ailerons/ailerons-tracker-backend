...
from geojson import Feature, GeoJSON, LineString, Point
...
from ailerons_tracker_backend.errors import (
    GeneratorLineError,
    GeneratorPointError,
)
...
class LineGeojson(Base):
    """Model for Line."""

    __tablename__ = "line_geojson"
    ...
    individual: Mapped["Individual"] = rel(back_populates="line_feature")
    geojson: Mapped[GeoJSON] = mc(postgresql.JSON)

    @classmethod
    def to_line_feature(
        cls, individual_records: list[Record], individual: Individual
    ) -> GeoJSON:
        ...
        properties = {
            ...
            "timestamps": list(
                map(
                    lambda record: str(record.record_timestamp),
                    individual_records,
                )
            ),
        }
        ...
        coordinates = list(
            map(
                lambda record: (record.longitude, record.latitude),
                individual_records,
            )
        )

        feature = Feature(
            geometry=LineString(coordinates), properties=properties
        )
